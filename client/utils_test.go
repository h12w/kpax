...
func testRequest(t *testing.T, request proto.Request, expected []byte) {
...
func decode(t *testing.T, response proto.Response, bytes []byte) {
	decoder := proto.NewBinaryDecoder(bytes)
	err := response.Read(decoder)
	assert(t, err, (*proto.DecodingError)(nil))
}

func decodeErr(t *testing.T, response proto.Response, bytes []byte, expected *proto.DecodingError) {
	decoder := proto.NewBinaryDecoder(bytes)
	err := response.Read(decoder)
