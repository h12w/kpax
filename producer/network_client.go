...
func (nc *NetworkClient) send(topic string, partition int32, batch []*ProducerRecord) {
	...
	if err != nil {
		for _, record := range batch {
			record.metadataChan <- &RecordError{Error: err}
		}
	}
	...
	if nc.requiredAcks > 0 {
		...
		for _, record := range batch {
			record.metadataChan <- &RecordError{
				Offset:    -1,
				Topic:     topic,
				Partition: partition,
...
func listenForResponse(topic string, partition int32, batch []*ProducerRecord, responseChan <-chan *connector.RawResponseAndError) {
	...
	if response.Err != nil {
		for _, record := range batch {
			record.metadataChan <- &RecordError{Error: response.Err}
		}
	}
	...
	if decodingErr != nil {
		for _, record := range batch {
			record.metadataChan <- &RecordError{Error: decodingErr.Error()}
		}
	}
